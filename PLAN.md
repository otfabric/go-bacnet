...
**v0.2.4** release notes ready (see [RELEASE.md](RELEASE.md)); cut via
Actions → Release → `patch`. Pin:
...
`v0.7.0` @ `224f51b` (Batches A–C evidence adapters; bacnet-stack **1.6.0**;
BACpypes3 **0.0.106**; Worldiety digest).

**Next cycle (evidence expansion — no new services):**

1. Cut **`v0.2.4`** (Release workflow `patch`); fill CI evidence links in RELEASE.md
2. Follow bacnet-interop Batch D (B5 network inventory) → pin bump when tagged
...
| Adapters | `bacnet-interop` `v0.7.0` (incl. Worldiety; File/lifecycle/NC/alarm/messaging/LSO/identity) |
...
| 3 | Pin file for bacnet-interop ref/digests; required pinned lane | done | `interop/bacnet-interop-pin.json` → `v0.7.0` |
