...
Pinned digests: [bacnet-interop v0.8.0](https://github.com/otfabric/bacnet-interop/releases/tag/v0.8.0)
...
| **BACnet4J** | Rejects segmented confirmed-request receive. GetEnrollmentSummary is the only live peer for that service at the pin. WriteGroup and COV-multiple not implemented. Who-Am-I / You-Are not available. |
| **bacnet-stack** | Broad native application surface. GetEnrollmentSummary, ConfirmedPrivateTransfer, TextMessage, and COV-multiple unsupported at 1.6.0. Who-Am-I / You-Are live. BBMD_ENABLED with self-seeded BDT / Accept-FD (FDR live). Write-BDT NAK at PR≥17. May Abort some segmented ComplexACK paths. Upstream router app not packaged. |
| **BACpypes3** | Strong COV (including renew) and segmented ASE. File / CreateObject / ReadRange / GetAlarmSummary / GetEnrollmentSummary unsupported at 0.0.106. Peer-as-BBMD via env. Some WPM / EventNotification / Reinit paths are adapter-assisted. |
